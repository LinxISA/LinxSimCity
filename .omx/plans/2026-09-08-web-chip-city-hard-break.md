# [Hard break] 将 LinxSimCity 重建为可建造、可仿真、可追踪 Tile 的 3D 芯片城市网页游戏

## 目标与决策

把 LinxSimCity 重建为面向芯片架构的浏览器单人建造与仿真沙盒。玩家从标准 3D 积木库组装芯片城市，运行受支持的 workload，观察 SimQueue 之间的事务流动和 Tile 的实际存储位置，再根据瓶颈调整配置并重新运行。

核心循环：**选择场景 → 建造/连接 → 校验 → 运行 → 观察与追踪 → 调整 → 对比新结果**。
...
- 首发面向桌面浏览器、键鼠、单人；支持空白沙盒、预制芯片蓝图、引导挑战和本地保存/导入/导出。
- 支持放置、网格吸附、XYZ 平移、90° 旋转、复制、框选、组合/解组、删除、撤销/重做及类型化端口连接。
- 默认轴测视角；支持自由相机、搜索定位、城区/模块/内部状态逐级展开、楼层切片与路径聚焦。
- XYZ 没有固定棋盘边界。采用整数空间分块与块内坐标、相机相对渲染、按需加载和 LOD；不承诺无限内存或无限同时可见物体。
- 展示布局与硬件装配图分离：仅移动外观不改变时序；修改硬件连接/参数生成新的模型配置。每次 run 绑定不可变的装配图与配置 hash，旧 trace 只能继续解释原 run，不能套在新配置上。
- 通用积木允许自由搭建；是否能运行由后端能力表决定。未支持的组合在运行前定位到具体组件/端口/参数，并保留建造成果，不伪造结果。
...
- 独立数据对象：组件定义、实例/端口连接图、展示布局、run manifest、初始状态/checkpoint、稀疏事件、蓝图存档。
...
- 静态网站支持完整建造、保存、挑战引导与已录制结果回放。真实“修改配置后重跑”首发通过本机 companion/runner 接受明确的配置和 workload；没有 runner 时清晰显示不可运行，不在 GitHub Pages 中假装执行本地模拟器。
...
- [ ] 提供至少一个新格式 synthetic bundle、一个蓝图和一个 schema 校验入口；更新 `tools/linxtrace`、`sdk/cpp` 的最小 writer。
...
### M3 — 可玩的 XYZ 建造工作空间（依赖 M1、M2）

- [ ] 用 `apps/game` 替换旧 viewer 主入口，新增 `packages/world`、`packages/editor`；同步更新 workspace、路由、构建和启动脚本。
- [ ] 实现组件库、吸附、放置、旋转、连接、框选、组合、复制、删除、撤销/重做、搜索与属性编辑。
- [ ] 完成三维分块、局部坐标、相机、切片/展开；交互不依赖有限大小的旧地板或固定城区坐标。
- [ ] 定义编辑命令和蓝图保存事务，支持本地自动保存、导出、导入和损坏存档诊断；结构编辑与展示编辑分别记录。

**完成条件：** 浏览器内从空白场景装配一条合法链路，保存后重载无身份/连接丢失；撤销/重做恢复同一结构 hash；在 XYZ 正负方向跨块编辑后仍能准确定位和拾取。
...
- [ ] 对 M0 选定的实际来源补充只读观测与新格式 producer adapter；先贯通一个受支持的后端，再验证第二个后端，不等待整个 DavinciOO 完成。
- [ ] 在真实模型存在的边界上贯通“请求入队 → 存储访问 → Tile 驻留 → 计算 → 写回”；数据缺口通过实际 instrumentation 补齐。
- [ ] 固定至少一个正常场景和一个确实产生队列反压或 bank conflict 的场景；保存源码/配置/workload hash、生成命令、事件计数与采集覆盖。
- [ ] 新站点首屏采用这一精品小场景；实现积木、时间轴、槽位/驻留和状态检查联动。

**完成条件：** 用户能在浏览器选择一条指令及其 Tile，定位真实等待原因和存储位置；采集开关不改变模型功能结果或模拟周期；页面所示关键状态由独立 trace/model 证据逐项核对。仅播放 synthetic 场景不能关闭本里程碑。

### M6 — 全目录城市与复合蓝图（依赖 M5）

- [ ] 将 240 项映射接入游戏组件库；实现 7 个城区、31 个子系统的可折叠层级、搜索、来源和能力标识。
- [ ] 补全可实例化模块的参数化外观与端口；contained-state、interface、alias、unresolved 按所属对象呈现。
- [ ] 支持参数化复合蓝图、多核/多 PE 复制及跨块连接；ID、owner 和状态不会因复制或展开冲突。
...
- [ ] 至少完成三个引导挑战：合法装配、定位队列瓶颈、分析并改善 bank conflict 或 Tile 搬运；每项有起始蓝图、可执行参数范围和独立完成判据。
...
- [ ] 浏览器 E2E 覆盖建造、保存重载、导入错误、run 失配、trace seek、Tile 定位、挑战闭环和 runner 断连；至少检查 Chromium、Firefox、Safari 的首发核心流程与降级说明。
...
| 渲染/编辑 | `packages/brick-kit`、`packages/scene-core`、`packages/world`、`packages/editor`、`apps/game`      | M2 → M3 → M5                 |
...
- [ ] 精制组件、XYZ 建造、存档、真实 trace、队列反压与 Tile 驻留在同一个网页产品内贯通。
...
| 任意搭建无法映射到真实模拟器          | 装配校验与可执行能力表分开；M7 只承诺明确支持的配置变换和 workload          |
